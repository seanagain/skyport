...
public final class SkyportConfig {
    ...
    public static final ModConfigSpec.IntValue UNATTENDED_MINUTES = SERVER
            .comment("How long a parked aircraft keeps starting new legs with no player near it.",
                     "",
                     "A parked aircraft normally releases its chunks and sleeps, so a route",
                     "only ever advances as far as its first stop unless somebody follows it",
                     "round - a loop never loops. This is how long it may carry on regardless:",
                     "it finishes its gate wait, sets off again, and keeps doing so until the",
                     "time runs out, holding its own chunks to do it.",
                     "",
                     "It bounds DEPARTURES, not flights. An aircraft already in the air always",
                     "finishes its leg and lands, however long that takes, because there is",
                     "nowhere to put an aeroplane down half way - letting the clock stop one",
                     "mid-route left it frozen over open country until somebody flew out to it.",
                     "A gate is somewhere an aircraft can wait indefinitely for nothing.",
                     "",
                     "The clock refills whenever a player comes within about 128 blocks, and on",
                     "engage, and when the tower wakes it. Spent, the aircraft stays at its gate",
